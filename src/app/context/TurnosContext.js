...
export const TurnosProvider = ({ children }) => {
  ...
  const [usuarioActual] = useState({
   ...
   const cargarTurnosUsuarioDesdeBackend = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/turnos/usuario/${usuarioActual.id}`
      );

      if (!response.ok) {
        console.error("No se pudieron obtener los turnos del usuario");
        return;
      }

      const data = await response.json();
      console.log("Turnos usuario desde backend:", data);
  ...
  useEffect(() => {
    cargarTurnosUsuarioDesdeBackend();
  }, []);
  ...
  const cancelarTurno = async (turnoId) => {
    ...
    return { ok: true };
     ...
     const obtenerTurnosProfesionalBackend = async (profesionalId) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/turnos?profesionalId=${profesionalId}`
      );

      if (!response.ok) {
        console.error("No se pudieron obtener los turnos del profesional");
        return;
      }

      const data = await response.json();

      const turnosConProfesional = data.map((turno) => ({
        ...turno,
        profesionalId,
      }));

      setTurnosReservados(turnosConProfesional);
    } catch (error) {
  ...
  return turnosReservados.filter((t) => t.usuarioId === usuarioId);
};
  ...
  const obtenerHorariosDisponiblesPorProfesional = (
   ...
   const value = {
    ...
    reservarTurno,
    cancelarTurno,
    obtenerTurnosProfesionalBackend,
